...
export const GithubCalendar = memo(function GithubCalendar({
  ...
  useEffect(() => {
    ...
    // Load the mini-game bundle only when Game Mode is toggled on. This keeps
    // ~500 lines of particle/bullet/rAF code out of the initial page bundle.
    let cleanup: (() => void) | null = null;
    let cancelled = false;
    import("./github-game").then(({ startContributionGame }) => {
      if (cancelled) return;
      cleanup = startContributionGame({
        canvas,
        weeks,
        data,
        step,
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-2 border-t border-border/60 pt-3">
          ...
          {showLegend && (
            <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground shrink-0 mt-0.5">
              ...
              {/* Game Mode Switch — desktop only (pointless on mobile, keeps
                  the game bundle from ever loading on phones) */}
              <div className="hidden lg:flex items-center gap-2 border-l border-border/80 pl-4">
